...
loginserver=$(az acr show -n $imageRegistry --query loginServer -o tsv)
echo "login server:" $loginserver

username=$(az acr credential show -n $imageRegistry --query "username" -o tsv)
echo "user name:" $username

password=$(az acr credential show -n $imageRegistry --query "passwords[0].value" -o tsv)
echo "password:" $password

resourcegroup=$(az acr show -n $imageRegistry --query "resourceGroup" -o tsv)
...
url=$(az container show -g $resourcegroup -n $imageName --query ipAddress.fqdn -o tsv)
echo "url:" $url
